...
Oyster filtration rates were estimated using the following formula from Ehrich & Harris \[Ehrich2015\]:

(1.3) 
$$
F = 0.17W^{0.75} 
$$ 
Where *F* is filtration rate in cubic meters per oyster per day, and *W* is body dry weight in g. Size at age of oysters in dry weight was interpolated from de Melo et al. \[deMelo2019\] which estimated mean dry weight of two-year-old oysters in Washington State as 4.8 g for *Magallana gigas*. Harvest weight was assumed to be reached at 4 years, and total estimates of oyster populations were equally grouped into age categories 1:4. Weights for each age class were assumed to be linear and were fit with the equation:

(1.4) $$
W_D = 2.4A
$$ 
Where *W~D~* is dry weight in grams and *A* is oyster age in years. Two population values were used to represent the oyster population size in Kachemak Bay. The first was based on interviews with oyster farmers in the area and produced a rough estimate of 1.5 million oysters within the bay. The second estimate is 975,745 oysters, based on a 2023 inventory reported to Alaska Fish and Game. The hourly filtration rate was calculated by dividing the daily rate by 24.
...
Filtration rate of oysters in cubic meters per hour in Jakolof Bay assuming 975,745 oysters:

```{r}
#| echo: false

pop_classes <- 975545/4
class_df <- data.frame(age = 1:4, population = pop_classes)
class_df <- class_df %>%
  mutate(dry_weight_ind = age * 2.4) %>%
  mutate(filter_rate_hourly = (0.17 * dry_weight_ind^0.75)/24) %>%
  mutate(total_filtration = filter_rate_hourly * population)

# total filtration in m^3
sum(class_df$total_filtration)

```
Filtration rate of oysters in cubic meters per hour in Jakolof Bay assuming 1.5 million oysters:

```{r}
#| echo: false

pop_classes <- 1500000/4
class_df <- data.frame(age = 1:4, population = pop_classes)
class_df <- class_df %>%
  mutate(dry_weight_ind = age * 2.4) %>%
  mutate(filter_rate_hourly = (0.17 * dry_weight_ind^0.75)/24) %>%
